...
|---|---|---|---|
| **Universal session controls / PTT** | `src/voice_translator/streaming/input_control.py`<br>`src/voice_translator/streaming/pipeline_outgoing.py` | `tests/unit/test_input_control.py`<br>`tests/unit/test_pipeline_ptt.py` | Release-only complete turns, timestamped key boundaries, 30-second bounded recording, pause-spanning acoustic evidence, pre-roll, cancellation. |
...
| **Machine Translation (MT)** | `src/voice_translator/translation/ctranslate_backend.py`<br>`src/voice_translator/translation/base.py` | `tests/unit/test_mt_ctranslate.py` | CTranslate2 MT (OPUS-MT TR↔EN, NLLB-200), EOS handling, glossary, whole-turn sentence preservation without prepended history. |
